# GP-function main
gp_main <- function(x_train, y_train, x_star, tau, phi, nu, distance_matrix_train,phi_vec, get_cov_star = FALSE) {
...
gp_main_sample <- function(x_train, y_train, x_star, tau,
                           nu, distance_matrix_train,phi_vec,
                           get_sample =  TRUE) {
...
# GP-function main
gp_main_slow <- function(x_train, y_train,
                         x_star, tau,
                         nu,
                         phi_vec,
                         get_sample =  FALSE) {
...
# GP-function main
gp_main_slow_no_noise <- function(x_train,
                                  y_train,
                                  x_star,
                                  nu,
                                  phi_vec,
  ...
  K_y <- kernel_function(
    squared_distance_matrix_phi = distance_matrix_K,
    nu = nu) + diag(x = 1e-12, nrow = n_train)
  K_diag <- is_diag_matrix(K_y)
  K_star <- kernel_function(squared_distance_matrix_phi =  distance_matrix_K_star,
                            nu = nu)
